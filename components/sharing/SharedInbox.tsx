...
import { Dumbbell, UtensilsCrossed, BookOpen, CheckCircle, ExternalLink, Loader2, Inbox } from 'lucide-react'
...
const TYPE_FILTERS = ['all', 'workout', 'saved_meal', 'recipe'] as const
...
export function SharedInbox() {
  ...
  async function handleImport(item: InboxItem) {
    ...
    try {
      ...
      setImportedIds((prev) => new Set([...prev, item.friend_share_id]))
    ...
    <div className="space-y-3">
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed border-border/60 rounded-xl">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">When friends share workouts, meals, or recipes, they&apos;ll appear here.</p>
        </div>
      ) : (
